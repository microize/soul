...
- **Q: How do I update Soul CLI to the latest version?**
  - A: If installed globally via npm, update Soul CLI using the command `npm install -g @google/gemini-cli@latest`. If run from source, pull the latest changes from the repository and rebuild using `npm run build`.

- **Q: Where are Soul CLI configuration files stored?**
...
- **Error: Command not found (when attempting to run Soul CLI).**
  - **Cause:** Soul CLI is not correctly installed or not in your system's PATH.
  - **Solution:**
    1.  Ensure Soul CLI installation was successful.
